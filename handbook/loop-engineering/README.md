...
把「直播 5 七步交付法」套进 Loop Engineering 框架：每一步的回路长什么样、出错往哪退。

七步是 **PRD → 设计 → 变更 → 执行 → 质量门 → 发布 → 复盘**。这份文档讲的是它底下的回路结构，也就是 `team-standards` 里 devloop、test-ready、kb 这几个动作背后的理论层。

- 主线闭环完整，每一步都有回退出口
- 执行段（第四步）是最容易被低估的一段，单独展开成第二张图
- 与仓库里的 `plugins/team-standards` 对得上：devloop = 计划回路 + TDD 回路 + 止损线，评审 = 防跑偏回路

图示为手绘 SVG，源文件在 `diagrams/`，PNG 是同尺寸 2× 导出，可直接贴进 PPT。
...
先说清楚：这里说的"漏"，是七步法这个**方法论**没覆盖的回路，不是说项目里缺这些。下面是"方法还能怎么补"。
...
**后四条本期都可以不做。** 主题是"从 0 到 1 交付"，做"从 1 到 N"再补。

## 四、一句话总结

> **Loop Engineering 是本课的理论框架，七步交付法是它在一套完整交付里的落地。主线闭环完整，每层都有回退出口；指标前置、可观测性、夜间基线、灰度、A/B 这几条，等做"从 1 到 N"再补。**

## 文件清单与改法

```
handbook/loop-engineering/
├── README.md                    # 本文档
└── diagrams/
    ├── main-loop.svg            # 主图，1440×800，手绘源文件
    ├── main-loop.png            # 同上，2× 导出
    ├── execution-loop.svg       # 执行回路图，1240×770
    ├── execution-loop.png       # 同上，2× 导出
    └── index.html               # 两张图的本地预览页
```

改文案或加减节点：直接编辑 `.svg` 里的 `<text>` 与 `<rect>` 坐标，改完用浏览器打开 `index.html` 核对，再重新导出 PNG。Mermaid 源码只作结构参照，不参与渲染。
